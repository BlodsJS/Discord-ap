...
class ChannelSystem:
    ...
    # ... manter mu00e9todos inserir/apagar/get_canais ...
    async def inserir(self, guild_id: int, channel_id: int, field: str) -> bool:
        """Adiciona canal a um campo especu00edfico (text/voice, thread)"""
        chave = str(guild_id)

        # Cria a estrutura se nu00e3o existir
        if chave not in self.dados:
            self.dados[chave] = {"text": [], "voice": [], "thread": []}
        ...
        if channel_id not in self.dados[chave][field]:
            ...
            await self._carregar_dados()
            return True
        return False

    async def apagar(self, guild_id: int, channel_id: int, field: str) -> bool:
        """Remove canal de um campo especu00edfico"""
        chave = str(guild_id)
